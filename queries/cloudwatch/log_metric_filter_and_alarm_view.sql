...
INNER JOIN aws_cloudwatchlogs_filters
           ON
        aws_cloudtrail_trails.cloudwatch_logs_log_group_name = aws_cloudwatchlogs_filters.log_group_name
INNER JOIN aws_cloudwatch_alarm_metrics
           ON
        aws_cloudwatchlogs_filters.name = aws_cloudwatch_alarm_metrics.metric_stat_metric_name
INNER JOIN
    aws_cloudwatch_alarms ON
        aws_cloudwatch_alarm_metrics.alarm_cq_id = aws_cloudwatch_alarms.cq_id
INNER JOIN
    aws_sns_subscriptions ON
        aws_sns_subscriptions.topic_arn = ANY(aws_cloudwatch_alarms.actions)
